                ...
                Else
                    ...
                    If Not MainTimer.Check(TimersIndex.Arrows, False) Then 'Check if arrows interval has finished.
                        ...
                        With FontTypes(FontTypeNames.FONTTYPE_TALK)
                            'Call AddtoRichTextBox(frmMain.RecChat, "No pod�s lanzar flechas tan rapido.", .red, .green, .blue, .bold, .italic)
                        End With
                        Exit Sub
                    End If
                    ...
                    If UsingSkill = Magia Then
                        If Not MainTimer.Check(TimersIndex.Attack, False) Then 'Check if attack interval has finished.
                            If Not MainTimer.Check(TimersIndex.CastAttack) Then 'Corto intervalo de Golpe-Magia
                                ...
                                UsingSkill = 0
                                With FontTypes(FontTypeNames.FONTTYPE_TALK)
                                   ' Call AddtoRichTextBox(frmMain.RecChat, "No puedes lanzar hechizos tan r�pido.", .red, .green, .blue, .bold, .italic)
                                End With
                                Exit Sub
                            End If
                        Else
                            If Not MainTimer.Check(TimersIndex.CastSpell) Then 'Check if spells interval has finished.
                                ...
                                UsingSkill = 0
                                With FontTypes(FontTypeNames.FONTTYPE_TALK)
                                    'Call AddtoRichTextBox(frmMain.RecChat, "No pod�s lanzar hechizos tan rapido.", .red, .green, .blue, .bold, .italic)
                                End With
                                Exit Sub
                            End If
                        End If
...
Private Sub Socket1_LastError(ErrorCode As Integer, ErrorString As String, Response As Integer)
    ...
    If ErrorCode = 24036 Then
        Call MsgBox("Si el servidor no le conecta en unos minutos, tiene problemas con su internet, por favor verif�quelo", vbApplicationModal + vbInformation + vbOKOnly + vbDefaultButton1, "Error")
        Exit Sub
