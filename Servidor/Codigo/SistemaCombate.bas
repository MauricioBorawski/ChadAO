...
Public Sub UserDa�oNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    With Npclist(NpcIndex)
        ...
        If da�o > 145 Then
            Call WriteChatOverHead(UserIndex, "�" & da�o & "!", UserList(UserIndex).Char.CharIndex, vbRed)
        Else
            Call WriteChatOverHead(UserIndex, da�o, UserList(UserIndex).Char.CharIndex, vbRed)
        End If
...
Public Sub NpcDa�o(ByVal NpcIndex As Integer, ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If da�o > 145 Then
            Call WriteChatOverHead(UserIndex, "�" & da�o & "!", Npclist(NpcIndex).Char.CharIndex, vbRed)
        Else
            Call WriteChatOverHead(UserIndex, da�o, Npclist(NpcIndex).Char.CharIndex, vbRed)
        End If
    ...
    With Npclist(Atacante)
        ...
        If Npclist(Victima).Numero = PRKING_NPC Then
            If pretorianosVivos > 0 Then
                Call WriteConsoleMsg(1, .MaestroUser, "Debes matar al resto del ej�rcito antes de atacar al rey!", FontTypeNames.FONTTYPE_FIGHT)
                .TargetNPC = 0
                Exit Sub
            End If
        End If
...
Public Sub UsuarioAtacaNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    If UserImpactoNpc(UserIndex, NpcIndex) Then
        ...
        If res < (UserList(UserIndex).Stats.UserSkills(eSkill.artes) / 4) Then
            ...
            Call WriteConsoleMsg(1, UserIndex, "Tu golpe ha dejado inm�vil a la criatura", FontTypeNames.FONTTYPE_INFO)
        End If
    Else
...
Public Sub UserDa�oUser(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer)
    ...
    With UserList(AtacanteIndex)
        ...
        If da�o > 145 Then
            Call WriteChatOverHead(AtacanteIndex, "�" & da�o & "!", UserList(AtacanteIndex).Char.CharIndex, vbRed)
            Call WriteChatOverHead(VictimaIndex, "�" & da�o & "!", UserList(AtacanteIndex).Char.CharIndex, vbRed)
        Else
            Call WriteChatOverHead(AtacanteIndex, da�o, UserList(AtacanteIndex).Char.CharIndex, vbRed)
            Call WriteChatOverHead(VictimaIndex, da�o, UserList(AtacanteIndex).Char.CharIndex, vbRed)
        End If
...
'***************************************************
    ...
    If UserList(attackerIndex).flags.Muerto = 1 Then
        Call WriteConsoleMsg(1, attackerIndex, "No pod�s atacar porque estas muerto", FontTypeNames.FONTTYPE_INFO)
        PuedeAtacar = False
        Exit Function
    End If
    ...
    If MapInfo(UserList(VictimIndex).Pos.map).Pk = False Then
        If esArmada(attackerIndex) Then
            If UserList(VictimIndex).Pos.map = 58 Or UserList(VictimIndex).Pos.map = 59 Or UserList(VictimIndex).Pos.map = 60 Then
                Call WriteConsoleMsg(1, VictimIndex, "Huye de la ciudad! estas siendo atacado y no podr�s defenderte.", FontTypeNames.FONTTYPE_WARNING)
...
'***************************************************
    ...
    If Npclist(NpcIndex).Hostile = 0 Then
        ...
        ElseIf Npclist(NpcIndex).MaestroUser = 0 Then
            ...
            If Not criminal(attackerIndex) Then
                ...
                If UserList(attackerIndex).flags.Seguro Then
                    Call WriteConsoleMsg(1, attackerIndex, "Para atacar a este NPC deb�s quitar el seguro", FontTypeNames.FONTTYPE_INFO)
                    PuedeAtacarNPC = False
                    Exit Function
                Else
                    Call WriteConsoleMsg(1, attackerIndex, "Atacaste un NPC No-Hostil. Continua haciendolo y ser�s Criminal.", FontTypeNames.FONTTYPE_INFO)
                    PuedeAtacarNPC = True
                    Exit Function
                End If
            End If
        ...
        Else
            ...
            If esCaos(Npclist(NpcIndex).MaestroUser) Then
                ...
                If esCaos(attackerIndex) Then
                    ...
                    Call WriteConsoleMsg(1, attackerIndex, "Los miembros de la Legi�n Oscura no pueden atacar mascotas de otros legionarios. ", FontTypeNames.FONTTYPE_INFO)
                    PuedeAtacarNPC = False
                    Exit Function
                End If
            End If
    ...
    If esPretoriano(NpcIndex) = 4 Then
        If pretorianosVivos > 0 Then
            Call WriteConsoleMsg(1, attackerIndex, "Debes matar al resto del ej�rcito antes de atacar al rey!", FontTypeNames.FONTTYPE_FIGHT)
            PuedeAtacarNPC = False
            Exit Function
        End If
    End If
